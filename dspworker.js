...
onmessage = function(event) {
    ...
    Module.onRuntimeInitialized = function() {
        ...
        postMessage({"type":"newTask", "message":"Initializing STL vectors"});
        let ret = {};
        ...
        postMessage({"type":"newTask", "message":"Computing spectrogram"});
        ...
        if (plotSpectrogram) {
            postMessage({"type":"newTask", "message":"Extracting maxes"});
        ...
        postMessage({"type":"newTask", "message":"Computing beepy tunes"});
        Module.jsGetBeepyTune(SEm, yEm, samples.length, timeWin, freqWin, win, hop, sr);
        ret.y = EMVec2Array(yEm);


        // Step 5: Free memory
        postMessage({"type":"newTask", "message":"Freeing memory"});
