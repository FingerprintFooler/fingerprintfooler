...
void jsGetSpectrogram(std::vector<double> sig, std::vector<std::vector<double>>& SOut, int N, int win, int hop, bool useWindow) {
  DSP dsp(win);
  int NWin;
  double** S = dsp.spectrogram((double*)&sig[0], N, win, hop, useWindow, &NWin);
  for (int i = 0; i < NWin; i++) {
